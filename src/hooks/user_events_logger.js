...
import NodePackagesABI from "../abi/NodePackagesABI1.json";
import NodeTokenABI from "../abi/NodeTokenABI1.json";

// Configuration
const PROVIDER_URL = import.meta.env.VITE_RPC_URL || "https://sepolia.base.org";
const CONTRACT_ADDRESS =
  import.meta.env.VITE_NODE_PACKAGES_ADDRESS ||
  "0x271d19C69fF93F9FaB2E35bcEb31A871A9d62657";
const TOKEN_ADDRESS =
  import.meta.env.VITE_NODE_TOKEN_ADDRESS ||
  "0x35024799A05Ed370CE0f8F9b803A5BC0c072E854";

class UserEventsLogger {
  constructor() {
    const effectiveProviderUrl = PROVIDER_URL;
    const effectiveContractAddress = CONTRACT_ADDRESS;
    const effectiveTokenAddress = TOKEN_ADDRESS;

    if (!effectiveProviderUrl) {
      throw new Error(
        "PROVIDER_URL is undefined. Ensure VITE_RPC_URL environment variable is set and accessible.",
      );
    }

    this.web3 = new Web3(effectiveProviderUrl);

    // Validate NodePackagesABI
    if (
      !NodePackagesABI ||
      typeof NodePackagesABI !== "object" ||
      NodePackagesABI === null
    ) {
      console.error(
        "NodePackagesABI import failed or is not an object:",
        NodePackagesABI,
      );
      throw new Error(
        "Failed to import NodePackagesABI or it is not a valid object. Check the file path and content of ../abi/NodePackagesABI.json",
      );
    }
    if (!Array.isArray(NodePackagesABI.abi)) {
      console.error(
        "NodePackagesABI.abi is not an array:",
        NodePackagesABI.abi,
      );
      throw new Error(
        'NodePackagesABI.abi is missing or is not an array. Ensure the JSON file has a valid "abi" array.',
      );
    }
    // The check for abi.length === 0 is in init(), which is fine as it's not about type error.

    // Validate NodeTokenABI
    if (
      !NodeTokenABI ||
      typeof NodeTokenABI !== "object" ||
      NodeTokenABI === null
    ) {
      console.error(
        "NodeTokenABI import failed or is not an object:",
        NodeTokenABI,
      );
      throw new Error(
        "Failed to import NodeTokenABI or it is not a valid object. Check the file path and content of ../abi/NodeTokenABI.json",
      );
    }
    if (!Array.isArray(NodeTokenABI.abi)) {
      console.error("NodeTokenABI.abi is not an array:", NodeTokenABI.abi);
      throw new Error(
        'NodeTokenABI.abi is missing or is not an array. Ensure the JSON file has a valid "abi" array.',
      );
    }
    ...
    try {
      ...
      console.error("Error creating Web3 contract instances:", e);
      throw new Error(`Failed to instantiate Web3 contracts: ${e.message}`);
    }
  ...
  async getAllUserEvents(
    ...
    try {
      ...
      try {
        ...
        for (
          ...
          try {
            ...
            const nodeEvents = await this.contract.getPastEvents(
              "allEvents",
              eventFilter,
            );
            ...
            if (nodeEvents.length > 0) {
              console.log(
                `✅ Found ${nodeEvents.length} NodePackages events in blocks ${start}-${end}`,
              );
            }
          } catch (chunkError) {
            console.log(
              `⚠️ Chunk ${start}-${end} failed, trying specific events...`,
            );
            ...
            const specificEvents = [
              "NodePurchased",
              "ReferralRegistered",
              "ReferralRegisteredAndRewardDistributed",
              "RewardsClaimed",
              "RewardsWithdrawn",
              "RewardWithdrawalRequest",
              "AddBoosterReward",
              "BulkReferralRewardEarned",
              "AdminMarketingBonusCollected",
              "ProsperityFundContribution",
              "ProsperityFundDistributed",
              "PackageProsperityFundContribution",
              "PackageProsperityFundDistributed",
              "LiquidityWithdrawn",
              "LiquidityWithdrawalSettingsUpdated",
              "DiscountedNodePurchased",
              "UserHoldReward",
              "UserReleaseReward",
            ];
            // array to reverse the order of events
            specificEvents.reverse();
        ...
        console.log(
          `⚠️ Warning: Could not fetch NodePackages events: ${error.message}`,
        );
      }
      ...
      try {
        ...
        const tokenEvents = await this.tokenContract.getPastEvents(
          "allEvents",
          tokenFilter,
        );
        ...
        console.log(
          `⚠️ Warning: Could not fetch Token events: ${error.message}`,
        );
        ...
        try {
          ...
          const transferEvents = await this.tokenContract.getPastEvents(
            "Transfer",
            transferFilter,
          );
          ...
          console.log(
            `⚠️ Could not fetch Transfer events: ${transferError.message}`,
          );
        }
      }
      ...
      allEvents.sort((a, b) => {
        const blockA =
          typeof a.blockNumber === "bigint"
            ? Number(a.blockNumber)
            : a.blockNumber;
        const blockB =
          typeof b.blockNumber === "bigint"
        ...
        const txA =
          typeof a.transactionIndex === "bigint"
            ? Number(a.transactionIndex)
            : a.transactionIndex || 0;
        const txB =
          typeof b.transactionIndex === "bigint"
  ...
  organizeEventsByUser(events) {
    ...
    console.log(
      `📋 Organized events for ${Object.keys(this.userEvents).length} unique users`,
    );
  }
  ...
  async addTimestampsToEvents() {
    ...
    for (const [userAddress, userEvents] of Object.entries(this.userEvents)) {
      ...
      if (processedUsers % 10 === 0) {
        console.log(
          `⏳ Processed ${processedUsers}/${Object.keys(this.userEvents).length} users...`,
        );
      }
    }
  ...
  generateSummaryReport() {
    ...
    const totalEvents = Object.values(this.userEvents).reduce(
    ...
    for (const events of Object.values(this.userEvents)) {
      for (const event of events) {
        allEventTypes[event.eventName] =
          (allEventTypes[event.eventName] || 0) + 1;
      }
    }

    report += `\n📈 Event Type Distribution:\n`;
    for (const [eventType, count] of Object.entries(allEventTypes).sort(
      (a, b) => b[1] - a[1],
    )) {
...
//   const isUserAddress =
//     args[0] && args[0].startsWith("0x") && args[0].length === 42;
...
// Export the class as default export for Vite
export default UserEventsLogger;
